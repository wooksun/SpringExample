...
public class HomeController {
	...
	//	HttpServletRequest 인터페이스 객체로 뷰 페이지에서 컨트롤러로 넘어오는 데이터를 받을 때 데이터가 넘어오지 않아도 에러가 
	//	발생되지 않고, null로 처리하지만 @RequestParam 어노테이션을 사용해서 뷰 페이지에서 넘어오는 데이터를 받을 때, 데이터가 넘어오지
	//	않으면 400 - Bad Request 에러가 발생된다.
	...
	public String memberInsert(HttpServletRequest request, Model model) {
		...
		//	뷰 페이지에서 post 방식으로 컨트롤러로 넘어와 HttpServletRequest 인터페이스 객체에 저장된 데이터를 받는다.
		...
		//	기본생성자로 VO 클래스의 객체를 만들고 setter 메소드로 데이터를 넣어준다.
...
//		vo.setEmail(email);
	...
	*/
	...
	//	커맨드 객체를 사용하면 HttpServletRequest 인터페이스 객체나 @RequestParam 어노테이션으로 넘겨받은 데이터를 클래스 객체에 
	//	setter 메소드를 사용해서 저장한 다음 Model 인터페이스 객체에 넣어주는 동작이 일괄적으로 처리된다.
	
	//	커맨드 객체는 뷰 페이지에서 컨트롤러로 넘어온 데이터를 저장할 클래스를 의미하며 뷰 페이지로 넘어가는 객체의 이름은
	//	커맨드 객체의 이름이 첫 문자만 소문자로 변경되서 넘어간다.
	...
	*/
	...
	@RequestMapping("/memberInsert")
	//	커맨드 객체의 이름이 너무 길어서 사용하기 불편하면 뷰 페이지로 넘겨주는 커맨드 객체 이름을 @ModelAttribute 어노테이션을
	//	이용해서 별도로 커맨드 객체의 이름을 지정해서 사용할 수 있다.
	//	이 경우, 기존 커맨드 객체 이름은 뷰 페이지에서 사용할 수 없고 @ModelAttribute 어노테이션으로 지정한 이름을 사용해야 한다.
	public String memberInsert(HttpServletRequest request,  Model model,
			/* MemberVO memberVO */
